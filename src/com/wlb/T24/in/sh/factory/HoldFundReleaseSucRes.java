...
public class HoldFundReleaseSucRes implements HoldFundRelease {
	...
	@Override
	public String retMsg(String outMsg, String filePath) {
		...
		String autRetMsg = autRetMsg(retMapMsg, filePath);
	...
	public HashMap<String, MapEntity> retMapMsg(String msg) {
		...
		for (int i = 0; i < testSLength; i++) {
			...
			// ------------------------------
			/*
			 * String[] data = testS[i].split("=", -1); map.put(data[0],
			 * data[1]);
			 */
	...
	public String getMMsgB(HashMap<String, List> retMapMsg, int i) {
		...
		String CRLimitSign = "+";
		ret = ccyCode + ledgerBalance + ledgerBalanceSign + availableBalance + availableBalanceSign + totalFloatAmount
				+ totalFloatAmountSign + totalHoldAmount + totalHoldAmountSign + CRLimit + CRLimitSign;
		return ret;
	}

	public String autRetMsg(HashMap<String, List> retMapMsg, String filePath) {
		String proTpCode = "";
		String propertyB = "";
		String mapString = "";
		String cycle = "";
		...
		int totalCycle = 0;
		int size = 0;
		Properties properties = Configuration.getProperites(filePath);
		int propertiesSize = properties.size();
		for (int i = 0; i < propertiesSize; i++) {
			// propertyB properties涓搴旂殑鍊�
			...
			if (splitLen == 4) {
				...
				if (retMapMsg.containsKey(valueSpe[0])) {
					...
					for (int k = 1; k < parseInt; k++) {
						String propertyC = properties.getProperty(String.valueOf(i + k));
						String[] valueSpeC = propertyC.split(":");
						if (retMapMsg.containsKey(valueSpeC[0])) {
							size = retMapMsg.get(valueSpeC[0]).size();
							String filedMsg = getFiledMsg(size, retMapMsg, valueSpeC, parseInt, i, properties);
							retMsgString += filedMsg;
						} else {
							for (int o = 0; o < Integer.parseInt(valueSpeC[2]); o++) {
								mapSpace = mapSpace + " ";
							}
				...
				// String propertyC =
				// properties.getProperty(String.valueOf(i+Integer.parseInt(cycle)+1));
				i += Integer.parseInt(cycle);
	...
	public String getFiledMsg(int size, HashMap<String, List> retMapMsg, String[] valueSpe, int parseInt, int i,
